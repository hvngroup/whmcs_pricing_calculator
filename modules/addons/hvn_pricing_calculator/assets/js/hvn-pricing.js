...
(function () {
    ...
    var Utils = {
        ...
        fmt: function (v) { if (v===-1) return '-1.00'; if (v===0) return '0.00'; return parseFloat(v).toFixed(2); },
        parse: function (s) { if (s===undefined||s===null||s==='') return null; var n=parseFloat(String(s).replace(/,/g,'').trim()); return isNaN(n)?null:n; },
        defaultCurrency: function () { if (!CFG.currencies) return null; for (var i=0;i<CFG.currencies.length;i++) { if (CFG.currencies[i].default==1) return CFG.currencies[i]; } return CFG.currencies[0]||null; },
        toast: function (msg, type) {
            type = type || 'info';
            var container = document.getElementById('hvn-toasts');
            if (!container) { container=document.createElement('div'); container.id='hvn-toasts'; container.className='hvn-toast-container'; document.body.appendChild(container); }
            var el = document.createElement('div');
            el.className = 'hvn-toast hvn-toast--' + type;
            el.textContent = msg;
            container.appendChild(el);
            requestAnimationFrame(function () { el.classList.add('hvn-toast--visible'); });
            setTimeout(function () { el.classList.remove('hvn-toast--visible'); setTimeout(function () { el.remove(); }, 300); }, 3500);
        },
        highlight: function (el) { el.classList.add('hvn-changed'); setTimeout(function () { el.classList.remove('hvn-changed'); }, 3000); },
        fetchJson: async function (url, opts) {
            opts = opts || {};
            opts.headers = Object.assign({ 'X-Requested-With': 'XMLHttpRequest' }, opts.headers || {});
            try { var resp = await fetch(url, opts); return await resp.json(); }
    ...
    var CYCLE_TO_SETUP = { quarterly:'qsetupfee', semiannually:'ssetupfee', annually:'asetupfee', biennially:'bsetupfee', triennially:'tsetupfee', monthly:'msetupfee' };

    /* ================================================================
       CALCULATOR ENGINE
       ================================================================ */
    var Calc = {
        MULTIPLIERS: { monthly:1, quarterly:3, semiannually:6, annually:12, biennially:24, triennially:36 },
        SETUP_MAP:   { monthly:'msetupfee', quarterly:'qsetupfee', semiannually:'ssetupfee', annually:'asetupfee', biennially:'bsetupfee', triennially:'tsetupfee' },

        cycles: function (baseVal, baseCycle, discounts, rounding, roundTo, enabledCycles) {
            if (baseVal===null||baseVal===-1) return null;
            var baseMult=this.MULTIPLIERS[baseCycle]||1, monthlyEquiv=baseVal/baseMult, result={};
            ...
            for (var cycle in this.MULTIPLIERS) {
                ...
                var raw=monthlyEquiv*mult, dk=dMap[cycle];
        ...
        setupFees: function (baseSetup, baseCycle, setupDiscounts, rounding, roundTo, enabledCycles) {
            ...
            var baseFeeKey=this.SETUP_MAP[baseCycle], result={};
            ...
            for (var cycle in this.SETUP_MAP) {
                ...
                var dk=dMap[feeKey], raw=baseSetup;
        ...
        convert: function (val, srcRate, tgtRate) { if (val===null||val===-1||val===0) return val; if (!srcRate||!tgtRate) return val; return val*(tgtRate/srcRate); }
    };
    ...
    /* ================================================================
       ...
       ================================================================ */
    var PricingDOM = {
        PRICE_IDX_MAP: { 1:'msetupfee',2:'qsetupfee',3:'ssetupfee',4:'asetupfee',5:'bsetupfee',11:'tsetupfee',6:'monthly',7:'quarterly',8:'semiannually',9:'annually',10:'biennially',12:'triennially' },
        _subOptionData: null,
        ...
        _parseStandardInputs: function (inputs) {
            var groups = {};
            inputs.forEach(function (el) { var m=el.name.match(/(?:currency|pricing)\[(\d+)\]\[(\w+)\]/); if (!m) return; var curId=parseInt(m[1]); if (!groups[curId]) groups[curId]={}; groups[curId][m[2]]=el; });
            return groups;
        },

        _parsePriceInputs: function (inputs) {
            var self=this, groups={}, bySubOption={};
            inputs.forEach(function (el) {
                var m=el.name.match(/price\[(\d+)\]\[(\d+)\]\[(\d+)\]/); if (!m) return;
                var curId=parseInt(m[1]),subId=parseInt(m[2]),cycle=self.PRICE_IDX_MAP[parseInt(m[3])]; if (!cycle) return;
            ...
            for (var curId in bySubOption) { groups[curId]={}; for (var subId in bySubOption[curId]) { for (var cycle in bySubOption[curId][subId]) { if (!groups[curId][cycle]) groups[curId][cycle]=bySubOption[curId][subId][cycle]; } break; } }
            return groups;
        },

        getSubOptionData: function () { return this._subOptionData || null; },
        readValues: function (curInputs) { var v={}; for (var f in curInputs) v[f]=Utils.parse(curInputs[f].value); return v; },
        writeValues: function (curInputs, vals, overwrite) {
            ...
            for (var f in vals) {
                ...
                curInputs[f].value=Utils.fmt(vals[f]); Utils.highlight(curInputs[f]); c++;
            }
            return c;
        }
    };
    ...
    var Undo = {
        _snap: null,
        save: function () { var data=[]; document.querySelectorAll('input[name*="currency["], input[name*="pricing["], input[name^="price["]').forEach(function (el) { data.push({el:el,val:el.value}); }); this._snap=data; },
        restore: function () { if (!this._snap||!this._snap.length) { Utils.toast('Nothing to undo.','warning'); return; } this._snap.forEach(function (s) { s.el.value=s.val; Utils.highlight(s.el); }); this._snap=null; Utils.toast('Undo successful.','info'); },
    ...
    function findDefaultPreset() {
        ...
        for (var i=0;i<CFG.presets.length;i++) { if (CFG.presets[i].is_default==1) return CFG.presets[i]; if (CFG.presets[i].name===CFG.defaultPreset) return CFG.presets[i]; }
    ...
    function getEnabledCycles(preset) {
        ...
        return {quarterly:preset.cycle_quarterly!=0,semiannually:preset.cycle_semiannually!=0,annually:preset.cycle_annually!=0,biennially:preset.cycle_biennially!=0,triennially:preset.cycle_triennially!=0};
    }

    /** Build toClear list from ec + baseCycle (shared by both toolbar components) */
    function buildClearList(ec, baseCycle) {
        var cfm={quarterly:'quarterly',semiannually:'semiannually',annually:'annually',biennially:'biennially',triennially:'triennially'};
        var sfm={quarterly:'qsetupfee',semiannually:'ssetupfee',annually:'asetupfee',biennially:'bsetupfee',triennially:'tsetupfee'};
        var list=[];
        for (var c in ec) { if (!ec[c]) { list.push(cfm[c]); list.push(sfm[c]); } }
        if (baseCycle==='annually') { list.push('monthly'); list.push('msetupfee'); }
        return list;
    }

    /* ================================================================
       ALPINE COMPONENT: Toolbar (native WHMCS pricing pages)
       ================================================================ */
    function toolbarData() {
        var dp=findDefaultPreset(), ec=getEnabledCycles(dp);
        return {
            ...
            presetId:dp?String(dp.id):'', presets:CFG.presets||[], currencies:CFG.currencies||[], showRates:CFG.showRates,
            dQ:dp?+dp.discount_quarterly:0, dSA:dp?+dp.discount_semiannually:5, dA:dp?+dp.discount_annually:10, dBi:dp?+dp.discount_biennially:15, dTri:dp?+dp.discount_triennially:20,
            sdQ:dp?+(dp.discount_setup_quarterly||0):0, sdSA:dp?+(dp.discount_setup_semiannually||0):5, sdA:dp?+(dp.discount_setup_annually||0):10, sdBi:dp?+(dp.discount_setup_biennially||0):15, sdTri:dp?+(dp.discount_setup_triennially||0):20,
            cQ:ec.quarterly, cSA:ec.semiannually, cA:ec.annually, cBi:ec.biennially, cTri:ec.triennially,

            loadPreset: function () {
                var s=this, p=this.presets.find(function (x) { return String(x.id)===String(s.presetId); }); if (!p) return;
                this.dQ=+p.discount_quarterly; this.dSA=+p.discount_semiannually; this.dA=+p.discount_annually; this.dBi=+p.discount_biennially; this.dTri=+p.discount_triennially;
                this.sdQ=+(p.discount_setup_quarterly||0); this.sdSA=+(p.discount_setup_semiannually||0); this.sdA=+(p.discount_setup_annually||0); this.sdBi=+(p.discount_setup_biennially||0); this.sdTri=+(p.discount_setup_triennially||0);
                ...
                this.baseCycle=p.base_cycle||'monthly'; this.rounding=p.rounding_method; this.roundTo=+p.rounding_precision;
            ...
            calcCycles: function () {
                var dc=Utils.defaultCurrency(); if (!dc) { Utils.toast('No default currency found.','error'); return; }
                var grps=PricingDOM.findInputs(); Undo.save(); var subData=PricingDOM.getSubOptionData(), cnt=0, ec=this._ec();
                if (subData) {
                    var curSubs=subData[dc.id]; if (!curSubs) { Utils.toast('No pricing inputs for default currency.','warning'); return; }
                    for (var subId in curSubs) {
                        var inp=curSubs[subId], bv=Utils.parse(inp[this.baseCycle]?inp[this.baseCycle].value:null);
                        if (bv===null||bv===-1) continue;
                        var cc=Calc.cycles(bv,this.baseCycle,this._d(),this.rounding,this.roundTo,ec); if (cc) cnt+=PricingDOM.writeValues(inp,cc,this.overwrite);
                        var bsk=Calc.SETUP_MAP[this.baseCycle], bs=Utils.parse(inp[bsk]?inp[bsk].value:null);
                        if (bs!==null&&bs!==-1&&bs!==0) { var sc=Calc.setupFees(bs,this.baseCycle,this._sd(),this.rounding,this.roundTo,ec); if (sc) cnt+=PricingDOM.writeValues(inp,sc,this.overwrite); }
                    }
                } else {
                    var inp=grps[dc.id]; if (!inp) { Utils.toast('No pricing inputs for default currency.','warning'); return; }
                    ...
                    var cc=Calc.cycles(bv,this.baseCycle,this._d(),this.rounding,this.roundTo,ec); cnt=cc?PricingDOM.writeValues(inp,cc,this.overwrite):0;
                    var bsk=Calc.SETUP_MAP[this.baseCycle], bs=Utils.parse(inp[bsk]?inp[bsk].value:null);
                    if (bs!==null&&bs!==-1&&bs!==0) { var sc=Calc.setupFees(bs,this.baseCycle,this._sd(),this.rounding,this.roundTo,ec); if (sc) cnt+=PricingDOM.writeValues(inp,sc,this.overwrite); }
                }
            ...
            calcCurrencies: function () {
                var dc=Utils.defaultCurrency(); if (!dc) { Utils.toast('No default currency found.','error'); return; }
                PricingDOM.findInputs(); if (!Undo.hasSnap()) Undo.save(); var subData=PricingDOM.getSubOptionData(), cnt=0, self=this, sr=+dc.rate;
                if (subData) {
                    var srcSubs=subData[dc.id]; if (!srcSubs) { Utils.toast('No pricing inputs for default currency.','warning'); return; }
                    this.currencies.forEach(function (cur) {
                        if (cur.id==dc.id) return; var tr=+cur.rate; if (!tr) { Utils.toast(cur.code+' rate=0, skipped.','warning'); return; }
                        var tgtSubs=subData[cur.id]; if (!tgtSubs) return;
                        for (var subId in srcSubs) {
                            if (!tgtSubs[subId]) continue;
                            var sv=PricingDOM.readValues(srcSubs[subId]), cv={};
                    ...
                    var grps=PricingDOM.findInputs(), si=grps[dc.id]; if (!si) { Utils.toast('No pricing inputs for default currency.','warning'); return; }
                    var sv=PricingDOM.readValues(si);
                    this.currencies.forEach(function (cur) {
                        if (cur.id==dc.id) return; var tr=+cur.rate; if (!tr) { Utils.toast(cur.code+' rate=0, skipped.','warning'); return; }
                        var ti=grps[cur.id]; if (!ti) return; var cv={};
                        for (var f in sv) { if (sv[f]===null||sv[f]===-1) continue; cv[f]=Utils.round(Calc.convert(sv[f],sr,tr),self.rounding,self.roundTo); }
            ...
            calcAll: function () { if (CFG.confirmApply&&!confirm('Apply pricing calculation to all fields?')) return; this.calcCycles(); this.calcCurrencies(); },
            undo: function () { Undo.restore(); },

            clearCycles: function () {
                var toClear=buildClearList(this._ec(),this.baseCycle);
                if (!toClear.length) { Utils.toast('No cycles to clear. Toggle off the cycles you want to reset.','warning'); return; }
                Undo.save(); var cnt=0, subData=PricingDOM.getSubOptionData();
                if (subData) {
                    for (var curId in subData) { for (var subId in subData[curId]) { var inp=subData[curId][subId]; toClear.forEach(function (f) { if (inp[f]&&Utils.parse(inp[f].value)!==-1) { inp[f].value='0.00'; Utils.highlight(inp[f]); cnt++; } }); } }
                } else {
                    var grps=PricingDOM.findInputs(); for (var curId in grps) { var inp=grps[curId]; toClear.forEach(function (f) { if (inp[f]&&Utils.parse(inp[f].value)!==-1) { inp[f].value='0.00'; Utils.highlight(inp[f]); cnt++; } }); }
                }
    ...
    function configManagerData() {
        return {
            loading:false, loaded:false, groups:[], currencies:CFG.currencies||[],
            activeCurrency:Utils.defaultCurrency()?Utils.defaultCurrency().id:null,
            existingGroups:[], selectedExistingGroup:'', newGroupName:'', newGroupDesc:'', saving:false,

            init: function () { if (CFG.page&&CFG.page.type==='product_edit') this._observeTab(); },

            _observeTab: function () {
                var self=this;
                var check=function () { var h=window.location.hash; if (h.indexOf('tab=5')!==-1||h.indexOf('tab5')!==-1) { if (!self.loaded&&!self.loading) self._loadData(); } };
                check(); window.addEventListener('hashchange',check);
                document.addEventListener('click',function (e) { var a=e.target.closest?e.target.closest('a'):null; if (a&&((a.textContent||'').indexOf('Configurable')!==-1||(a.getAttribute('href')||'').indexOf('tab=5')!==-1)) setTimeout(check,300); });
            },

            _loadData: async function () {
                this.loading=true; var pid=CFG.page?CFG.page.product_id:0;
            ...
            getPricing: function (sub,curId,field) { if (!sub.pricing||!sub.pricing[curId]) return '-1.00'; var v=sub.pricing[curId][field]; return v!==undefined&&v!==null?v:'-1.00'; },
            setPricing: function (sub,curId,field,val) { if (!sub.pricing) sub.pricing={}; if (!sub.pricing[curId]) sub.pricing[curId]={}; sub.pricing[curId][field]=val; },
            optionTypeLabel: function (t) { return {'1':'DROPDOWN','2':'RADIO','3':'YESNO','4':'QUANTITY'}[String(t)]||'TYPE-'+t; },

            quickCreate: async function () {
                if (!this.newGroupName.trim()) { Utils.toast('Name required.','warning'); return; } this.saving=true;
                var r=await Utils.fetchJson(CFG.ajaxUrl+'&action=quick_create_group',{method:'POST',body:new URLSearchParams({name:this.newGroupName.trim(),description:this.newGroupDesc.trim(),product_id:CFG.page?CFG.page.product_id:0}),headers:{'Content-Type':'application/x-www-form-urlencoded'}});
            ...
            assignGroup: async function (gid) {
                ...
                var r=await Utils.fetchJson(CFG.ajaxUrl+'&action=assign_group',{method:'POST',body:new URLSearchParams({group_id:gid,product_id:CFG.page?CFG.page.product_id:0}),headers:{'Content-Type':'application/x-www-form-urlencoded'}});
            ...
            savePricing: async function () {
                this.saving=true; var payload=[],ho=[],hs=[];
                this.groups.forEach(function (g) { g.options.forEach(function (opt) {
                    ...
                    opt.subs.forEach(function (sub) {
                        ...
                        for (var cid in sub.pricing) {
                            ...
                            ['monthly','quarterly','semiannually','annually','biennially','triennially','msetupfee','qsetupfee','ssetupfee','asetupfee','bsetupfee','tsetupfee'].forEach(function (f) { if (p[f]!==undefined) rec[f]=p[f]; });
                            payload.push(rec);
                        }
                    });
                }); });
                var postData={pricing:JSON.stringify(payload),hidden_options:JSON.stringify(ho),hidden_subs:JSON.stringify(hs)};
                var token=CFG.token||''; if (!token) { var inp=document.querySelector('input[name="token"]'); if (inp) token=inp.value; } if (token) postData.token=token;
                var r=await Utils.fetchJson(CFG.ajaxUrl+'&action=save_config_options',{method:'POST',body:new URLSearchParams(postData),headers:{'Content-Type':'application/x-www-form-urlencoded'}});
    ...
    function configToolbarData() {
        ...
        return {
            ...
            calcCycles: function () {
                var m=this._mgr(); if (!m) { Utils.toast('Manager not ready.','warning'); return; } this._snap();
                ...
                m.groups.forEach(function (g) { g.options.forEach(function (opt) { opt.subs.forEach(function (sub) {
                    ...
                    var bv=Utils.parse(p[base]);
                    if (bv!==null&&bv!==-1) { var cc=Calc.cycles(bv,base,self.discounts,self.rounding,self.roundTo,ec); if (cc) for (var c in cc) { if (c===base) continue; var cv=Utils.parse(p[c]); if (cv===-1) continue; if (!self.overwrite&&cv!==null&&cv!==0) continue; p[c]=Utils.fmt(cc[c]); cnt++; } }
                    var bsk=Calc.SETUP_MAP[base], bs=Utils.parse(p[bsk]);
                    if (bs!==null&&bs!==-1&&bs!==0) { var sc=Calc.setupFees(bs,base,self.setupDiscounts,self.rounding,self.roundTo,ec); if (sc) for (var sk in sc) { if (sk===bsk) continue; var sv=Utils.parse(p[sk]); if (sv===-1) continue; if (!self.overwrite&&sv!==null&&sv!==0) continue; p[sk]=Utils.fmt(sc[sk]); cnt++; } }
                }); }); });
                m.groups=m.groups.slice(); Utils.toast('Config cycles: '+cnt+' fields.','success');
            },

            calcCurrencies: function () {
                var m=this._mgr(); if (!m) { Utils.toast('Manager not ready.','warning'); return; } if (!this._undoSnap) this._snap();
                var dc=Utils.defaultCurrency(); if (!dc) { Utils.toast('No default currency.','error'); return; }
                var sid=dc.id, sr=+dc.rate, cnt=0, self=this;
                var af=['monthly','quarterly','semiannually','annually','biennially','triennially','msetupfee','qsetupfee','ssetupfee','asetupfee','bsetupfee','tsetupfee'];
                m.groups.forEach(function (g) { g.options.forEach(function (opt) { opt.subs.forEach(function (sub) {
                    if (!sub.pricing||!sub.pricing[sid]) return; var sp=sub.pricing[sid];
                    m.currencies.forEach(function (cur) {
                        if (cur.id==sid) return; var tr=+cur.rate; if (!tr) return;
                        if (!sub.pricing[cur.id]) sub.pricing[cur.id]={}; var tp=sub.pricing[cur.id];
                        af.forEach(function (f) { var sv=Utils.parse(sp[f]); if (sv===null||sv===-1) return; var tv=Utils.parse(tp[f]); if (tv===-1) return; if (!self.overwrite&&tv!==null&&tv!==0) return; tp[f]=Utils.fmt(Utils.round(Calc.convert(sv,sr,tr),self.rounding,self.roundTo)); cnt++; });
                    });
                }); }); });
            ...
            clearCycles: function () {
                var toClear=buildClearList(this._ec(),this.baseCycle);
                if (!toClear.length) { Utils.toast('No cycles to clear.','warning'); return; }
                var m=this._mgr(); if (!m) { Utils.toast('Manager not ready.','warning'); return; }
                this._snap(); var cnt=0;
                m.groups.forEach(function (g) { g.options.forEach(function (opt) { opt.subs.forEach(function (sub) {
                    for (var cid in sub.pricing) { if (!sub.pricing[cid]) continue; var p=sub.pricing[cid]; toClear.forEach(function (f) { if (p[f]!==undefined&&Utils.parse(p[f])!==-1) { p[f]='0.00'; cnt++; } }); }
                }); }); });
                m.groups=m.groups.slice(); Utils.toast('Cleared '+cnt+' field(s). Use Undo to revert.','info');
            }
        };
    }
    ...
    function presetManagerData() {
        return {
            ...
            _empty: function () { return {id:0,name:'',base_cycle:'monthly',discount_quarterly:0,discount_semiannually:0,discount_annually:0,discount_biennially:0,discount_triennially:0,discount_setup_quarterly:0,discount_setup_semiannually:0,discount_setup_annually:0,discount_setup_biennially:0,discount_setup_triennially:0,cycle_quarterly:true,cycle_semiannually:true,cycle_annually:true,cycle_biennially:true,cycle_triennially:true,rounding_method:'round',rounding_precision:1,is_default:false}; },
            ...
            edit: function (p) {
                ...
                ['cycle_quarterly','cycle_semiannually','cycle_annually','cycle_biennially','cycle_triennially'].forEach(function (k) { f[k]=(f[k]==1||f[k]===true); });
                this.form=f; this.editing=p.id;
            },
            cancel: function () { this.editing=null; },
            save: async function () {
                if (!this.form.name||!this.form.name.trim()) { Utils.toast('Name required.','warning'); return; }
                this.saving=true; var formData=Object.assign({},this.form);
                formData.is_default=formData.is_default?'1':'0';
                ['cycle_quarterly','cycle_semiannually','cycle_annually','cycle_biennially','cycle_triennially'].forEach(function (k) { formData[k]=formData[k]?'1':'0'; });
                ...
                if (r.success) { Utils.toast('Saved!','success'); this.editing=null; await this.load(); } else Utils.toast('Error: '+(r.error||''),'error');
                this.saving=false;
            },
            remove: async function (id) {
                if (!confirm('Delete?')) return;
                var r=await Utils.fetchJson(CFG.ajaxUrl+'&action=delete_preset',{method:'POST',body:new URLSearchParams({id:id}),headers:{'Content-Type':'application/x-www-form-urlencoded'}});
    ...
    function initConfigManager() {
        ...
        var attempt=function () {
            ...
            var tab=findConfigTab(); if (!tab) return;
            var mount=document.createElement('div');
            mount.id='hvn-config-mount'; mount.setAttribute('x-data','hvnConfigManager()'); mount.setAttribute('x-init','init()'); mount.className='hvn-config-mgr hvn-mt-12';
            mount.innerHTML=buildConfigManagerHTML(); tab.appendChild(mount); injected=true;
            if (window.Alpine&&window.Alpine.initTree) window.Alpine.initTree(mount);
        };
        var onNav=function () { var h=window.location.hash; if (h.indexOf('tab=5')!==-1||h.indexOf('tab5')!==-1) setTimeout(attempt,300); };
        onNav(); window.addEventListener('hashchange',onNav);
        document.addEventListener('click',function (e) { var a=e.target.closest?e.target.closest('a'):null; if (a&&((a.textContent||'').indexOf('Configurable')!==-1||(a.getAttribute('href')||'').indexOf('tab=5')!==-1)) setTimeout(onNav,200); });
    }
    ...
    function buildConfigManagerHTML() {
        ...
        h+='<div class="hvn-assign-row"><select x-model="selectedExistingGroup" class="hvn-select" style="flex:1;height:32px"><option value="">Select group...</option><template x-for="g in existingGroups" :key="g.id"><option :value="g.id" x-text="g.name"></option></template></select>';
        ...
        h+='<div x-data="hvnConfigToolbar()" class="hvn-toolbar hvn-mb-8">'+buildEmbeddedToolbar()+'</div>';
        ...
        ['monthly','quarterly','semiannually','annually','biennially','triennially'].forEach(function (c) { h+='<td :class="{\'hvn-cell-disabled\':getPricing(sub,activeCurrency,\''+c+'\')==\'-1.00\'}"><input type="text" :value="getPricing(sub,activeCurrency,\''+c+'\')" @change="setPricing(sub,activeCurrency,\''+c+'\',$event.target.value)" class="hvn-config-input"></td>'; });
        ['msetupfee','qsetupfee','ssetupfee','asetupfee','bsetupfee','tsetupfee'].forEach(function (f) { h+='<td><input type="text" :value="getPricing(sub,activeCurrency,\''+f+'\')" @change="setPricing(sub,activeCurrency,\''+f+'\',$event.target.value)" class="hvn-config-input"></td>'; });
    ...
    function buildEmbeddedToolbar() {
        ...
        [['cQ','Quarterly'],['cSA','Semi-Annual'],['cA','Annual'],['cBi','Biennial'],['cTri','Triennial']].forEach(function (d) { h+='<label class="hvn-cycle-toggle hvn-cycle-toggle--inline" :class="{\'hvn-cycle-toggle--active\':'+d[0]+'}"><input type="checkbox" x-model="'+d[0]+'" class="hvn-cycle-toggle__checkbox"><span class="hvn-cycle-toggle__check" x-text="'+d[0]+'?\'✓\':\'✕\'"></span>'+d[1]+'</label>'; });
        h+='</div></div></div>';
    ...
    function getToolbarInnerHTML() {
        ...
        h+='</div></div></div>';
        h+='<div class="hvn-toolbar-row"><div class="hvn-discounts"><label class="hvn-discounts__label">Discounts:</label><div class="hvn-discounts__fields">';
        [['dQ','Quarterly','cQ'],['dSA','Semi-Annual','cSA'],['dA','Annual','cA'],['dBi','Biennial','cBi'],['dTri','Triennial','cTri']].forEach(function (d) { h+='<div class="hvn-discount" :class="{\'hvn-form-discount--disabled\':!'+d[2]+'}"><label>'+d[1]+'</label><input type="number" x-model.number="'+d[0]+'" min="0" max="100" step="0.5" :disabled="!'+d[2]+'"><span class="hvn-pct">%</span></div>'; });
        h+='</div></div></div>';
        h+='<div class="hvn-toolbar-row"><div class="hvn-discounts"><label class="hvn-discounts__label">Setup Fee:</label><div class="hvn-discounts__fields">';
        [['sdQ','Quarterly','cQ'],['sdSA','Semi-Annual','cSA'],['sdA','Annual','cA'],['sdBi','Biennial','cBi'],['sdTri','Triennial','cTri']].forEach(function (d) { h+='<div class="hvn-discount" :class="{\'hvn-form-discount--disabled\':!'+d[2]+'}"><label>'+d[1]+'</label><input type="number" x-model.number="'+d[0]+'" min="0" max="100" step="0.5" :disabled="!'+d[2]+'"><span class="hvn-pct">%</span></div>'; });
        h+='</div></div></div>';
        h+='<div class="hvn-toolbar-row"><div class="hvn-actions">';
        h+='<button type="button" class="hvn-btn hvn-btn--primary hvn-btn--sm" @click="calcCycles()">📊 Calc Cycles</button>';
        h+='<button type="button" class="hvn-btn hvn-btn--primary hvn-btn--sm" @click="calcCurrencies()">💱 Calc Currencies</button>';
    ...
    function injectToolbarForPopup() {
        var tryInject=function () {
            ...
            var target=document.querySelector('input[name^="price["]'); if (!target) return;
            var container=target.closest('table')||target.closest('form'); if (!container) return;
            var div=document.createElement('div');
            div.id='hvn-toolbar-mount'; div.setAttribute('x-data','hvnToolbar()'); div.innerHTML=getToolbarInnerHTML();
            container.parentNode.insertBefore(div,container);
            if (window.Alpine&&window.Alpine.initTree) window.Alpine.initTree(div);
        };
        setTimeout(tryInject,100); setTimeout(tryInject,500); setTimeout(tryInject,1500);
